...
import "./interfaces/IMieRouter02.sol";
...
contract MieRouter is IMieRouter02 {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = MieLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint256 amountBOptimal = MieLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, "MieRouter: INSUFFICIENT_B_AMOUNT");
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = MieLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function removeLiquidity(
        ...
        (uint256 amount0, uint256 amount1) = IMiePair(pair).burn(to);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = MieLibrary.sortTokens(input, output);
            IMiePair pair = IMiePair(MieLibrary.pairFor(factory, input, output));
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountOutput = MieLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint256 amount0Out, uint256 amount1Out) = input == token0
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, MieLibrary.pairFor(factory, path[0], path[1]), amountIn);
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            "MieRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    }
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(MieLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(amountOut >= amountOutMin, "MieRouter: INSUFFICIENT_OUTPUT_AMOUNT");
    ...
    function getAmountIn(
        ...
        return MieLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    }
